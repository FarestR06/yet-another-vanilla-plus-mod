...
public class NetherReactorCoreBlock extends Block {
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (isStructureCorrect(world, pos)) {
            ...
            BlockState activatedCore = state.with(PHASE, Phase.ACTIVE);
            if (player.hasStatusEffect(StatusEffects.BAD_OMEN)) {
                int badOmenLevel = Objects.requireNonNull(
                        player.getStatusEffect(StatusEffects.BAD_OMEN)
                ).getAmplifier();
                ...
                world.setBlockState(pos, activatedCore.with(OMINOUS, true));
