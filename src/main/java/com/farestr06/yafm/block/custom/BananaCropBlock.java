...
public class BananaCropBlock extends CropBlock {
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        if (ctx != null) {
            int amount = 1;
            if (random.nextFloat() < 0.17) amount++;
            this.tryGrow(world, ctx.state, ctx.pos, amount);
        }
    }

    private void tryGrow(ServerWorld world, BlockState state, BlockPos pos, int amount) {
        int i = Math.min(state.get(AGE) + amount, 5);
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        return (!isLowerHalf(state) || canPlaceAt(world, pos)) && super.canPlaceAt(state, world, pos);
    }
    ...
    private BananaCropBlock.BottomContext getLowerHalfContext(WorldView world, BlockPos pos, BlockState state) {
        if (isLowerHalf(state)) {
            ...
            return isLowerHalf(blockState) ? new BottomContext(blockPos, blockState) : null;
        }
    }
